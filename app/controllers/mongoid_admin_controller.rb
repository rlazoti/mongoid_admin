class MongoidAdminController < ApplicationController
  ...
  def build_klasses
    unless defined? $mongoid_admin_klasses
      model_dir = File.join(RAILS_ROOT,'app','models')
      model_names = Dir.chdir(model_dir) { Dir["**/*.rb"] }
      klasses = get_klasses(model_names)
      $mongoid_admin_klasses = keep_only_mongoid_klasses(klasses).sort_by {|r| r.name.underscore}
    end
    @klasses = $mongoid_admin_klasses
  end
  ...
  def keep_only_mongoid_klasses(klasses)
    klasses.select { |k| k.include?(Mongoid::Document) }
  end
